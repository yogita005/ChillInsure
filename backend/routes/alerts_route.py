...
async def get_risk_alerts(
    ...
    try:
        ...
        # Get city from pincode, fallback to zone-to-city mapping
        city = get_city_from_pincode(user_pincode)
        if city == "Your Zone" and user_zone in ZONE_TO_CITY:
            city = ZONE_TO_CITY[user_zone]
        elif city == "Your Zone":
            # Last resort: extract city from zone name (e.g. "koramangala_bengaluru" -> "Bengaluru")
            parts = user_zone.split("_")
            city = parts[-1].capitalize() if parts else "Bengaluru"
        
        # Get zone risk — try pincode first, then default based on zone
        ...
        return {
            ...
            "alerts": alerts,
            "weather": disruption_data.get("weather_data"),
            "aqi": disruption_data.get("aqi_data")
        }
